...
public class CNode implements Serializable {
	...
	public Instances transform(Instances D) throws Exception {
		...
		if(this.inX!=null) { //delete attributes selected
		Remove          remove;
	     Instances inst   = new Instances(D);
	     remove = new Remove();
	     remove.setAttributeIndicesArray(this.inX);;
	     remove.setInputFormat(inst);
	     D = Filter.useFilter(inst, remove);
	     D.setClassIndex(L);
		}
	...
	public void build(Instances D, Classifier H) throws Exception {
		...
		String[] parts = H.getClass().toString().split(" ");
	...
	public Instance transform(Instance x, double ypred[]) throws Exception {
		...
		x.setDataset(null);
		if(this.inX!=null) {
			for(int e =inX.length-1;e>=0;e--) {
				x.deleteAttributeAt(this.inX[e]);
			}
		}
	...
	*/
	...
	public double classify(Instance x, double ypred[]) throws Exception {
		...
		//System.out.println(x.numClasses());
		Instance x_ = transform(x,ypred);
	...
	/**
	 * Returns the underlying fil.
	 *
	 * @return the fil
	 */
	public Classifier getFil() {
		return fil;
	}
