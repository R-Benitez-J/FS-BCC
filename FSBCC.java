...
public class FSBCC extends CC {
	...
	public void buildClassifier(Instances D) throws Exception {
		...
		//evaluador y busqueda de atributos
		//WrapperSubsetEval eval = new WrapperSubsetEval();
		CfsSubsetEval eval = new CfsSubsetEval();
		//CorrelationAttributeEval eval = new CorrelationAttributeEval();
		//BestFirst search = new BestFirst();
		if(this.getSearchType().compareTo("BF")==0) {
			System.out.println(this.getSearchType());
			BestFirst search = new BestFirst();
		...
		for(int j : m_Chain) {
			...
			filter.SelectAttributes(D_j);

			int [] indices = filter.selectedAttributes();

			System.out.print("Atributos para Clase "+j);
			System.out.println(" Num atributo -1: "+(indices.length-1));
			D_j=new Instances(D);

			D_j.setClassIndex(L-1);
			boolean borrar=true;
			int [] inEli=null;
			inEli = new int [d-indices.length+1];
			int agre=0;
			if(inEli.length>1) {
				for (int quita=1;quita<=d;quita++) {

					if(Arrays.binarySearch(indices,quita)<0) {
						inEli [agre]=quita+L-1;
						agre++;
					}
			...
			int[] attributes = new int[indices.length-1];
			for(int k=0;k<attributes.length;k++) {
				attributes[k]=indices[k]+L-1;
			}
			if(inEli!=null)
				System.out.println("\t node h_"+j+" : P(y_"+j+" | x_["+Utils.arrayToString(attributes)+"], y_"+Arrays.toString(paL[j])+")");
			nodes[j] = new CNode(j,inEli, paL[j]);
	...
	String m_SearchType= "BF";

	public String searchTypeTipText() {
		return "Method to search the subset of features \n"
				+ "BF: BestFirst' (default) \n"
				+ "GS: GreedyStepwise \n";
	}
	...
	public void setOptions(String[] options) throws Exception {
		...
		setSearchType(OptionUtils.parse(options, 'S', "BF"));
	...
	public String [] getOptions() {
		...
		OptionUtils.add(result, 'S', getSearchType());
		OptionUtils.add(result, super.getOptions());
		return OptionUtils.toArray(result);
	}

	public static void main(String args[]) {
		ProblemTransformationMethod.evaluation(new FSBCC(), args);
	}
}
